...
import { workerInstance } from './utils/worker';
import { downloadTxtFile } from './utils/txtGenerator';
...
export default function SLC() {
  ...
  const [maxTarget, setMaxTarget] = useState(100);
  const [isLoading, setIsLoading] = useState(false);

  const workerCall = useCallback(async (hashedEmails: string[], emailsList: string[], isLoadingFinish: boolean = false, max?: number) => {
    
      if(isLoadingFinish){
        setIsLoading(true);
        const result = await workerInstance.suppressionListWorker(hashedEmails, emailsList, isLoadingFinish, max);
        setIsLoading(false);
        console.log("result", result);
        // result?.positiveEmails?.length?setCleanEmails(prev=>prev.concat(result?.positiveEmails)):null;
        result?.positiveEmails?.length?setCleanEmails(result?.positiveEmails):null;
      }
      workerInstance.suppressionListWorker(hashedEmails, emailsList, isLoadingFinish, max);
  ...
  const handleSaveEmails = () => {
    if(cleanEmails.length){
      downloadTxtFile(cleanEmails, "cleaned-emails.txt");
    }
    else{
  ...
  const [hashedEmailsFile, setHashedEmailsFile] = useState();
  const [hashedemailsCounter, setHashedEmailsCounter] = useState(0);
  const [emailsFile, setEmailsFile] = useState();
  const [emailsCounter, setEmailsCounter] = useState(0);
  const [cleanEmails, setCleanEmails] = useState([]);

  const handleDrop = (e:any, hashedOnes: boolean = false) => {
    const CHUNK_SIZE = 1024 * 1024;
      ...
      reader.onload = (event: any) => {
        ...
        if (hashedOnes) {
            const cleanFormat = textChunk.split('\n').map(line => line.trim())
            console.log("hashed emails to process length : ",cleanFormat.length);
            setHashedEmailsCounter(prev=>prev+cleanFormat.length);
            setHashedEmailsFile(cleanFormat.slice(0, 10));
            workerCall(cleanFormat, [])
        } else {
          const cleanFormat = textChunk.split('\n')
              .map(line => line.trim())
              .filter(line => line.includes('@') && line.includes('.'))
          console.log("emails to process length : ",cleanFormat.length);
          setEmailsCounter(prev=>prev+cleanFormat.length);
          setEmailsFile(cleanFormat.slice(0, 10));
          workerCall([], cleanFormat);
        }
        ...
        readNextChunk();
          ...
          // if(hashedOnes){
          //     setHashedEmailsFile(text);
          //     setHashedEmailsToProcess(text.split('\n').map(line => line.trim()));
          //     console.log("Hashed emails", text.split('\n').map(line => line.trim()));
          // }else{
          //     setEmailsFile(text);
          //     setEmailsToProcess(text.split('\n')
          //     .map(line => line.trim())
          //     .filter(line => line.includes('@') && line.includes('.')));
          //     console.log("Emails", text.split('\n')
          //     .map(line => line.trim())
          //     .filter(line => line.includes('@') && line.includes('.')));
          // }
      };
                ...
                <div className="flex w-full items-center mb-2">
                  <div className='flex'>
                    <div className="text-left">
                        <div className="text-xs text-gray-400 dark:text-gray-400 mb-2">Supression list cleaner</div>
                    </div>
                  </div>
              ...
              <div className="sm:p-4 p-4">
                {/* <div className='flex pb-3'>
                  <div className='flex w-full'>
                    <div className="text-xs text-gray-400 dark:text-gray-400 text-left nowrap w-full ms-2 center">Mail list</div>
                    <div className="text-xs text-gray-400 dark:text-gray-400 text-left nowrap w-full ms-2 center">Supression list</div>
                    <div className='flex w-full'>
                    <div className="text-xs text-gray-400 dark:text-gray-400 text-left nowrap w-full ms-2 center">Cleaned data</div>
                      <button onClick={generate} className="anime-2 bg-blue px-4 py-1 text-white rounded-md flex items-center justify-center" style={{textWrap:"nowrap"}}>Clean data</button>
                    </div>
                  </div>
                </div> */}
                <div className='flex'>

                  <div className='colv w-full'>
                    <div className='cardv center p-4 bg-gray-200 dark:bg-gray-800  border border-gray-300 dark:border-gray-700 dark:text-white w-full rounded-md text-xs'
                    onDrop={handleDrop}
                    ...
                    <h3>Drag & Drop .txt file here</h3>
                      {emailsFile && (
                        <div>
                          <div className='w-full mt-3' style={{display:"inline-grid", overflow:"hidden"}}>
                            {emailsFile.map((email, index) => (
                              <span className='bg-gray-200 dark:bg-gray-900 p-1' style={{borderRadius:".5rem", margin:".1rem"}} key={index}>{email}</span>
                            ))}
                          </div>
                          <h4 className='mt-3'>Total Records : {emailsCounter} emails</h4>
                        </div>
                      )}
                    </div>
                    ...
                    <textarea value={emailResult} onChange={(e) => setRmailResult(e.target.value)}  placeholder="email@mail.ex" className="p-4 cardv bg-transparent border border-gray-300 dark:border-gray-700 dark:text-white w-full rounded-md text-xs"></textarea>
                  </div>

                  <div className='colv w-full mx-4'>
                    <div className='cardv center p-4 bg-gray-200 dark:bg-gray-800  border border-gray-300 dark:border-gray-700 dark:text-white w-full rounded-md text-xs'
                    onDrop={(event: any)=>{ handleDrop(event, true) }}
                    onDragOver={handleDragOver}
                    style={{textAlign: 'center',alignItems: "center"}}>
                      <h3>Drag & Drop .txt file here</h3>
                      {hashedEmailsFile && (
                        <div>
                          <div className='w-full mt-3' style={{display:"inline-grid", overflow:"hidden", maxHeight: "calc(20vh)"}}>
                            {hashedEmailsFile.map((email, index) => (
                              <span className='bg-gray-200 dark:bg-gray-900 p-1' style={{borderRadius:".5rem", margin:".1rem", padding: "0.25rem 0.75rem"}} key={index}>{email}</span>
                            ))}
                          </div>
                          <h4 className='mt-3'>Total Records : {hashedemailsCounter} emails</h4>
                        </div>
                      )}
                    </div>
                    <div className='p-4 center'>
                  ...
                  <div className='colv w-full'>
                  <div className='mb-2 font-medium flex justify-between items-center'> <span>total : {cleanEmails.length}</span> <span>Max:  <input type="number" value={maxTarget} onChange={(e)=>{ setMaxTarget(parseInt(e.target.value)) }} placeholder='target number' className='p-2 rounded-md text-xs border-[0.1px] border-gray-500 bg-[#18181b]' /> </span> </div>
                    <textarea value={cleanEmails.join('\n')} onChange={(e) => setRmailResult(e.target.value)} placeholder="Cleaned data" className="p-4 cardv bg-transparent border border-gray-300 dark:border-gray-700 dark:text-white w-full rounded-md text-xs"></textarea>
                    <div className='flex flex-between gap-2 justify-between mt-4 mb-1'>
                      { !isLoading && <button onClick={handleStartProcessing} className="anime-2 text-lg font-medium bg-green-600 px-4 py-2 text-white rounded-md flex items-center justify-center flex-grow-[2]" style={{textWrap:"nowrap"}}>start</button> }
                      { isLoading && <button onClick={handleStartProcessing} disabled className="anime-2 text-lg font-medium bg-green-600 px-4 py-2 text-white rounded-md flex items-center justify-center flex-grow-[2]" style={{textWrap:"nowrap"}}>loading...</button> }
                      <button onClick={handleSaveEmails} className="text-lg font-medium bg-blue-700 px-4 py-2 text-white rounded-md flex items-center justify-center flex-grow-1" style={{textWrap:"nowrap"}}>save</button>
                    </div>
                  </div>
                </div>
              </div>
